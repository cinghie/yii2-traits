...
- Refreshed runtime dependency constraints where appropriate for the PHP 8.1 baseline, including Yii 2.0.55, Cocur Slugify 4.7.1, Google Cloud Translate 2.3.3 and PHP-FFMpeg 1.4.x.
- Kept `dektrium/yii2-user` and `cinghie/yii2-user-extended` on `dev-master` because the Cinghie module ecosystem depends on their current branch behavior.
...
- `dev-master` dependencies are intentionally retained for Dektrium User and Cinghie User Extended; they are validated by the release CI but remain moving branch references rather than immutable tagged releases.
